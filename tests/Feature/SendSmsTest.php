...
use ToneflixCode\KudiSmsNotification\Tests\Notifications\SendCorpSms;
...
test('user can recieve corporate sms', function () {
    $user = User::factory()->create();

    expect(fn() => $user->notify(new SendCorpSms("Hello {$user->name}! This is test SMS.")))
        ->not()
        ->toThrow(\Exception::class);
})->skip('Skipped for cost saving.');
